...
class Application {
    ...
    constructor(applicationHolder, jsonData = {}) {
        ...
        /** default atlas value is ho-cb */
        this.atlasChoice = "ho-cb"
    }
    /**
    ...
    async addButton() {
        ...
        // create the atlas chooser
        this.atlasDiv = document.createElement("div")
        this.atlasDiv.id = "atlasDiv"
        this.atlasDiv.innerHTML = "Atlas Options:"
        this.applicationHolder.append(this.atlasDiv)

        /** Harvard Oxford Cortical Brain atlas selection button */
        this.HO_CB_btn = document.createElement("button")
    ...
    /**
     * Switch the atlas being used to draw the boundaries, requires tolling out the existing panes, so will show an alert before continuing.
     * @memberof Application
     */
    async atlasSwap() {
        ...
        this.addPane()
        // update the atlas type in the panes
    }
    /**
    ...
    async sessionRecreator(importData) {
        for (let pane of importData.panes) {
            // check what the atlas is , if its different than the application change it, should only get called once tops
            if (pane.atlasChoice != this.atlasChoice) {
                await this.HCP_btn.click()
            }
            console.log(pane)
            await this.addPane()
...
class divMaker {
    ...
    constructor(width, holder) {
        ...
        this.element = d
        // use this value in the updateFillFilter
        this.width = width
        d.style.height = "30px"
        d.style.width = "30px"
